...
if (isset($_POST['reg'])) {
    if (!empty($_POST['firstname']) &&  !empty($_POST['lastname']) && !empty($_POST['users'])  && !empty($_POST['email']) && !empty($_POST['password']) ) {
        ...
        $check_query = "SELECT * FROM adminlogin WHERE user = '$username' OR email = '$email'";
        ...
        if (mysqli_num_rows($check_result) > 0) {
            ...
            // Insert new user
            $sql = "INSERT INTO adminlogin (firstname, lastname, user, email, password) VALUES ('$firstname', '$lastname', '$username', '$email', '$password')";
