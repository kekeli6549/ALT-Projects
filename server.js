...
app.post('/api/upload', auth, upload.single('file'), async (req, res) => {
    // If file filter rejected, req.file will be undefined and req.file will also be undefined if no file selected.
    if (!req.file) {
    ...
    try {
        ...
        // Multer errors (e.g., file size limit) are usually thrown by multer itself.
        // If the error comes from multer.single, it might be caught by the general error handling middleware (if you had one).
        // For now, catch specific errors here, otherwise it's a generic 500
...
// @access  Private (requires authentication)
app.get('/api/my-files', auth, (req, res) => { // <--- Ensure this line is app.get and the path is correct
    try {
        const files = readFiles();
        const userFiles = files.filter(file => file.uploader === req.user); // req.user comes from JWT
...
app.get('/api/all-files', auth, (req, res) => {
    try {
        const files = readFiles(); // Read all file metadata
        // For MVP, return all files. In a real app, you might add pagination, sorting, or filtering.
